"""
Working version, will need to remove all the database items from the __init__, 
and create class and methods.
"""
...
class DataBase():
    ...
    def __init__(self):
        ...
        self.cur.execute(f''' CREATE TABLE  if not exists rtbhigh(playername text, score integer) ''')
        self.cur.execute(f''' CREATE TABLE if not exists finisheshigh(playername text, score integer) ''')
 
        self.connection.commit()

    def updateDb(self, tablename, playername, score):
        self.cur.execute(f'''INSERT INTO {tablename} (playername, score) VALUES (?,?)''', (playername, score))
        self.connection.commit()


    def displayDb(self, tablename):
        self.cur.execute(f'SELECT * FROM {tablename}')
        rtb = self.cur.fetchall()
        return rtb
...
class RtbPage(tk.Frame):
    ...
    def gameFinished(self):
        ...
        saveButton = tk.Button(frame3, text="Save", command=self.updatedatabase)
        saveButton.grid(row=0, column=2, sticky='nsew')
 
    def updatedatabase(self):
        db.updateDb("rtbhigh", self.name, self.runningTotal)
        print("rtb done")
...
class FinishesPage(tk.Frame,):
    ...
    def updatedatabase(self):
        db.updateDb("finisheshigh", self.name, self.runningTotal)
        print("done")
...
class HighScoresPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        items = db.displayDb("rtbhigh")
        print(items)
 
        tk.Label(frame1, text="Round the board", bg='lightgray', highlightbackground="black", highlightthickness=1).grid(row=0,column=0, columnspan=3)
        ...
        items = db.displayDb("finisheshigh")
        print(items)
 
        tk.Label(frame2, text="Finishes",bg='lightgray').grid(row=0,column=0, columnspan=3)
